...
begin
  ...
  begin
    ...
    try
      ExtractTemporaryFile('GetNetworkInterfaces.ps1');
      
      ScriptFile := ExpandConstant('{tmp}\GetNetworkInterfaces.ps1')
      NetworkListFile := ExpandConstant('{tmp}\network_interfaces.txt')

      // Run PowerShell script and capture output
      if Exec('powershell.exe', '-ExecutionPolicy Bypass -File "' + ScriptFile + '"', ExpandConstant('{tmp}'), SW_HIDE, ewWaitUntilTerminated, ResultCode) then
      begin
        if LoadStringsFromFile(ExpandConstant('{tmp}\network_interfaces.txt'), Lines) then
        begin
...
begin
  ...
  begin
    ...
    InitializeBitmapImageFromIcon(IconImage, ExpandConstant('{tmp}\network.ico'), Page.SurfaceColor, [32, 48, 64]);
    ...
    begin
      ...
      FNpcapLibraryLabel.Caption  := 'Npcap library will be installed, to allow capturing of raw pakets.';
...
begin
  if ShouldConfigureInsomnia then
    Result := IsComponentSelected('InsomniaService\NetworkMonitor') and not HasMultipleNetworkMonitors()
  else
    Result := False;
end;
